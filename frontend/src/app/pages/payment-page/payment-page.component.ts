    ...
    <div class="max-w-2xl mx-auto px-6 py-12 animate-in slide-in-from-top-4 duration-700">
      <div class="glass-card !p-10">
        <div class="flex items-center gap-4 mb-10">
           ...
           <h2 class="text-3xl font-black">Secure Payment</h2>
        </div>
        ...
        <form [formGroup]="paymentForm" (ngSubmit)="processPayment()" class="space-y-10">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label 
              ...
              [class.border-slate-800]="paymentForm.get('payment_method')?.value !== 'transfer'"
            >
              <input type="radio" formControlName="payment_method" value="transfer" class="hidden">
              <span class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Method 01</span>
              <span class="text-lg font-bold">Bank Transfer</span>
            </label>
            ...
            <label 
              ...
              [class.border-slate-800]="paymentForm.get('payment_method')?.value !== 'credit_card'"
            >
              <input type="radio" formControlName="payment_method" value="credit_card" class="hidden">
              <span class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Method 02</span>
              <span class="text-lg font-bold">Credit Card</span>
            </label>
          </div>

          <!-- Bank Info (Conditional) -->
          <div class="bg-slate-900 border border-slate-800 rounded-2xl p-8 space-y-4 animate-in fade-in zoom-in-95" *ngIf="paymentForm.get('payment_method')?.value === 'transfer'">
            <div class="flex justify-between pb-4 border-b border-slate-800">
              <span class="text-slate-500">Bank Name</span>
              <span class="font-bold">VinylBank (VBK)</span>
            </div>
            <div class="flex justify-between pb-4 border-b border-slate-800">
              <span class="text-slate-500">Account Name</span>
              <span class="font-bold uppercase">VinylLab Co., Ltd.</span>
            </div>
            <div class="flex justify-between">
              <span class="text-slate-500">Account No.</span>
              <span class="font-mono text-xl text-indigo-400 font-bold">123-4-56789-0</span>
            </div>
          </div>
          ...
          <div class="space-y-6 animate-in fade-in transition-all" *ngIf="paymentForm.get('payment_method')?.value === 'credit_card'">
            ...
            <div class="grid grid-cols-2 gap-4">
               <div class="space-y-2">
                 <label class="text-sm font-medium text-slate-400">Expiry</label>
