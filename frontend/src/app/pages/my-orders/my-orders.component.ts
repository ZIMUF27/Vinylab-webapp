  ...
  template: `
    <div class="max-w-7xl mx-auto px-6 py-12 animate-in fade-in duration-500">
      <div class="flex justify-between items-end mb-12">
        <div>
          <h1 class="section-title mb-2">Order History</h1>
          <p class="text-slate-400">Track and manage your custom sign orders</p>
        </div>
        <div class="text-xs font-bold text-slate-500 italic">Showing all orders</div>
      </div>
          ...
          <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-white/5">
              <tr *ngFor="let order of orders()" class="hover:bg-white/5 transition-colors group">
                <td class="px-8 py-6 font-mono text-xs text-indigo-400">#{{ order.id.slice(0, 8) }}</td>
                <td class="px-8 py-6">
                  <div class="font-bold text-slate-200">{{ order.design.template.name }}</div>
                  <div class="text-xs text-slate-500 mt-1 italic">{{ order.design.width }} x {{ order.design.height }} cm • {{ order.design.template.material_type }}</div>
                </td>
                <td class="px-8 py-6">
                  <span class="inline-flex items-center px-4 py-1.5 rounded-full text-xs font-bold ring-1 ring-inset" [ngClass]="getStatusClass(order.status)">
                ...
                <td class="px-8 py-6 text-slate-400 text-sm italic">{{ order.created_at | date:'MMM d, y, h:mm a' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <ng-template #empty>
        <div class="glass-card !p-20 text-center flex flex-col items-center gap-6">
          <div class="w-20 h-20 bg-slate-900 rounded-full flex items-center justify-center text-slate-700">
          ...
          <div>
            ...
            <p class="text-slate-500">You haven't designed or ordered any signs yet.</p>
          </div>
